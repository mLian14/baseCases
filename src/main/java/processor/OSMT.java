...
public class OSMT {
    ...
    public OSMT(String path, String outputPath) throws FileNotFoundException {
    ...
    public void processToOutputIIC(Document document){
        ...
        for (int i = 1; i <= node_cnt; ++i) {
            for (int j = 1; j <= node_cnt; ++j) {
                double deltaX = Math.abs(sub_BD(nodes.get(i - 1).getX_exact(), nodes.get(j - 1).getX_exact()));
                double deltaY = Math.abs(sub_BD(nodes.get(i - 1).getY_exact(), nodes.get(j - 1).getY_exact()));

                d[i][j] = Math.ceil(Math.sqrt(2) * Math.min(deltaX, deltaY) + Math.abs(deltaX - deltaY));
        ...
        d[0][0] = 0.0;
        for (int i = 1; i <= node_cnt; ++i) {
            double deltaX = Math.abs(nodes.get(i - 1).getX_exact() - master.getX_ct());
            double deltaY = Math.abs(nodes.get(i - 1).getY_exact() - master.getY_ct());

            d[i][0] = d[0][i] = Math.ceil(Math.sqrt(2) * Math.min(deltaX, deltaY) + Math.abs(deltaX - deltaY));
    ...
    private void octSteinerNodesGenerator(ArrayList<Node> set, ArrayList<Node> nodes, Node ms_node) {
        for (Node tn : set) {
            nodes.add(tn);
        }
        ...
        for (int i = 0; i < set_copy.size(); ++i) {
            ...
            for (int j = i + 1; j < set_copy.size(); ++j) {
                if (j != i) {
                    ...
                    steinNodes[2] = new Node(x1, y2, NodeType.SteinerNode);
                    ...
                    for (Node steinNode : steinNodes) {
                        if (!set_copy.contains(steinNode) || !nodes.contains(steinNode)) {
                            ...
                            System.out.println("This Steiner Point is in Terminals/Contained");
                        }
    ...
    public double dist(Point p1, Point p2){
        ...
        double deltaY = Math.abs(p1.y - p1.y);
        return Math.ceil(Math.min(deltaX, deltaY) + Math.abs(deltaX - deltaY));
    }
